...
X.s <- apply(X[,c(11, 14, 15, 17)], 2, scale) #' just the continuous ones

X.scaled <- cbind(X[,1:10], X.s[,1],
                  X[,12:13], X.s[,2:3],
                  X[,16], X.s[,4])
colnames(X.scaled)
colnames(X.scaled)[c(11,16,17)] <- c("maternal_age", "male", "gest_age_w")
...
load(here::here("Results", "BKMR_Model_BW.rdata"))
...
ggplot(pred.resp.univar.50, aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) + 
  ...
  facet_wrap(~ variable) +
  ylab("BW h(z) holding other variables at 50th percentile")
ggsave(filename = here::here("Figs", "BW h(x) holding other vars at 50th.jpeg"),
...
ggsave(filename = here::here("Figs", "BW h(x) holding other vars at 25th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggsave(filename = here::here("Figs", "BW h(x) holding other vars at 75th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggsave(filename = here::here("Figs", "BW h(x1, x2) holding other vars at 50th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggplot(pred.resp.bivar.25, aes(z1, z2, fill = est)) + 
  ...
  ylab("expos2") +
  ggtitle("h(expos1, expos2)")
ggsave(filename = here::here("Figs", "BW h(x1, x2) holding other vars at 25th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggplot(pred.resp.bivar.75, aes(z1, z2, fill = est)) + 
  ...
  ylab("expos2") +
  ggtitle("h(expos1, expos2)")
ggsave(filename = here::here("Figs", "BW h(x1, x2) holding other vars at 75th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggplot(pred.resp.bivar.levels.50, aes(z1, est)) + 
  ...
  xlab("expos1")
ggsave(filename = here::here("Figs", "BW h(x1, quant x2) holding other vars at 50th.jpeg"),
...
ggplot(risks.overall, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) + 
  ...
  geom_pointrange()
ggsave(filename = here::here("Figs", "BW Overall risk relative to 50th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggplot(risks.singvar, aes(variable, est, ymin = est - 1.96*sd, 
  ...
  coord_flip()
ggsave(filename = here::here("Figs", "BW Individual risk 25th to 75th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
...
ggplot(risks.int, aes(variable, est, ymin = est - 1.96*sd, 
  ...
  geom_pointrange(position = position_dodge(width = 0.75)) + 
  coord_flip()
ggsave(filename = here::here("Figs", "BW Interactions 25th to 75th.jpeg"),
       device = "jpeg", height = 5, width = 5, units = "in")
